...
import { GENERATE_RANDOM_DECKS, MINIMUM_DECK_SIZE } from '../core/constants.js';
...
export const runGame = async () => {
    ...
    let testActiveDeck: CardDefinition[] = Array.from({ length: MINIMUM_DECK_SIZE }, (_, i) => activeCardPool[i % activeCardPool.length]);
    let testPassiveDeck: CardDefinition[] = Array.from({ length: MINIMUM_DECK_SIZE }, (_, i) => passiveCardPool[i % passiveCardPool.length]);
    ...
    const ivoryDeck: GameDeck = {
        ...
        leader: getLeaderDefinition('leader_pakal_ritual'),
    };
